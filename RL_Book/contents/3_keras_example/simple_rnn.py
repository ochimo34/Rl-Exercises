...
def preprocessing(texts, labels):
	tokenizer = Tokenizer(vocab_size) # 枠作り
	tokenizer.fit_on_texts(texts) # ベクトル化
...
def build_rnn_model():
	model = Sequential()
	model.add(Embedding(vocab_size, 2, input_length=max_length, mask_zero=True))
...
# 学習
def train(model, train_padded_docs, train_labels, epochs=1000):
	return model.fit(train_padded_docs, train_labels, epochs=epochs, verbose=0)
